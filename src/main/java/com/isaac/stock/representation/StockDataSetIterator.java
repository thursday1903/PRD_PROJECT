...
import com.isaac.stock.dao.TblStockHistoryHome;
...
public class StockDataSetIterator implements DataSetIterator {
	...
	private final Map<PriceCategory, Integer> featureMapIndex = ImmutableMap.of(PriceCategory.OPEN, 0,
			PriceCategory.CLOSE, 1, PriceCategory.LOW, 2, PriceCategory.HIGH, 3, PriceCategory.VOLUME, 4);
	...
	public StockDataSetIterator(String filename, String symbol, int miniBatchSize, int exampleLength, double splitRatio,
		...
		List<StockData> stockDataList = readStockDataFromDB(symbol);
//				readStockDataFromFile(filename, symbol);
		...
		for (int i = 0; i < stockDataList.size() - window; i++) {
			...
			for (int j = i; j < i + exampleLength; j++) {
				...
				input.putScalar(new int[] { j - i, 4 },
						(stock.getVolume() - minArray[4]) / (maxArray[4] - minArray[4]));
			}
	...
	private List<StockData> readStockDataFromDB(String symbol) {
		List<StockData> stockDataList = new ArrayList<>();
		try {
			for (int i = 0; i < maxArray.length; i++) { // initialize max and min arrays
				maxArray[i] = Double.MIN_VALUE;
			...
			try {
				list_ = tblStockHistoryHome.listAllObject(new TblStockHistory());
			} catch (Exception e) {
			...
			for (Object object : list_) {
				TblStockHistory tblStockHistory = (TblStockHistory) object;
				String[] strTmp = new String[7];
				strTmp[0] = tblStockHistory.getRegDate();
				strTmp[1] = tblStockHistory.getStockCode();
				strTmp[2] = tblStockHistory.getOpenPrice().toString();
				strTmp[3] = tblStockHistory.getClosePrice().toString();
				strTmp[4] = tblStockHistory.getLowPrice().toString();
				strTmp[5] = tblStockHistory.getHighPrice().toString();
				strTmp[6] = tblStockHistory.getVolume().toString();
				list.add(strTmp);
